...
Component({
    ...
    ready() {
        ...
        // 无规格产品
        console.log('----- popup init `````', this.data)
        if (this.data.goodsInfo.spec_list.length < 1) {
            this.setData({
                goodsNoSpec: true
            })
        }
        console.log('----- popup after check`````', this.data)
        // 无规格产品 --- end  ---
    ...
    methods: {
        ...
        onConfirmClickNoSpec() {
            console.log('huasuan---')
            if(this.data.userInfo!==null){
                this.triggerEvent('confirm-click-nospec')
                console.log('trigger nospec click')
            }
        },
        onConfirmClick() {
            if(this.data.userInfo!==null){
