...
export default class Login {
    ...
    async _wechatLogin(data) {
        ...
        if (token) {
            ...
            const user_info = await userModel.self(token)
            fa.cache.set('user_info', user_info)

            // save wechat user info
            let wechatUserData = {
                nickname: data.wechat_username,
                avatar: data.wechat_avatar
            }

            fa.cache.set('user_info_wechat', wechatUserData)
                // 回调
